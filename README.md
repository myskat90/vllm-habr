...
- **Развертывание Kubernetes-кластера**: Шаги по установке и настройке Kubernetes. Как добавить новые узлы с GPU в существующий кластер.
- **Использование Deckhouse для расширения кластера**: Подключение Deckhouse для автоматического добавления узлов с GPU.
- **Использование KubRay**: Установка KubRay для управления GPU-ресурсами в Kubernetes.

#### Часть 3: Настройка vLLM для шардирования и распределения вычислений
- **Что такое vLLM**: Объяснение, почему для шардирования модели и распределения вычислений между несколькими GPU был выбран vLLM.
- **Распределение инференса**: Подробное описание того, как vLLM использует NCCL для распределения вычислений по нескольким GPU и как это работает.
- **Настройка шардирования модели**: Инструкции по шардированию модели и распределению вычислений на несколько GPU.

#### Часть 4: Настройка Ray Serve для вывода API
- **Что такое Ray Serve**: Объяснение, как Ray Serve позволяет масштабировать инференс и обрабатывать запросы через API.
- **Развертывание Ray Serve с FastAPI**: Пошаговое руководство по развертыванию Ray Serve и настройке совместимого с OpenAI API для получения предсказаний от модели.
- **Подключение Ray Serve к Kubernetes**: Как настроить Ray Serve в Kubernetes-кластере для распределения нагрузки.
...
- **Интеграция с OpenWebUI**: Как интегрировать Ray Serve и vLLM с OpenWebUI для мониторинга и управления задачами.
