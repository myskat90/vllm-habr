...
#### Часть 4: Настройка KubeRay Cluster и запуск инференса
- **Подготовка Docker-образа и сохранение его в registry**  
  Описание базового образа Ray, добавление скриптов `serve.py` и `auth.py`, установка зависимостей (vLLM и пр.), сборка и пуш в реестр.
- **Настройка KubeRay cluster**  
  Пример `values.yaml` для Helm-чарта: конфигурация Ray Head и Worker (с GPU), примонтирование CephFS, ресурсы (CPU/GPU), переменные окружения и прочие параметры.
- **Развёртывание Ray Application**  
  Определение JSON с импортом нашего сервиса (`serve:model`), установка переменных окружения (MODEL_ID), шардирование (tensor/pipeline parallel).
- **Тестирование инференса**  
  Проверка доступности ChatCompletion-эндпоинта (JWT-аутентификация, вызов с токеном), анализ ответов и производительности.
- **Мониторинг в Ray Dashboard**  
  Отслеживание нагрузки на GPU, просмотр логов и контроль общего состояния запущенных приложений.

#### Часть 5: Настройка авторизации и интеграция с OpenWebUI
- **Интеграция с OpenWebUI**: Разворачивание OpenWebUI в качестве интерфейса взаимодействия с LLM и его настройкой с Ray Application API. Использование JWT для аутентификации пользователей и защиты API
- **Вывод приложения в интернет**: Подключение приложения через CDN и настройка защиты с помощью WAF для безопасного доступа.
- **Примеры запросов**: Пример работы с OpenWebUI для генерации текста с использованием настроенной инфраструктуры.
...
- **Развёртывание Kubernetes-кластера с Deckhouse**  
  С помощью Deckhouse мы создали Kubernetes-кластер, добавили GPU-узлы и интегрировали инструменты, такие как NVIDIA GPU Operator и KubeRay Operator, для эффективного управления вычислительными ресурсами.
...
- **Подготовка Docker-образа и настройка KubeRay Cluster**  
  Мы подробно описали процесс сборки Docker-образа на основе официального Ray-образа, добавления наших скриптов и необходимых зависимостей, а затем развертывание Ray-кластера с помощью Helm-чарта. Это позволило создать оптимизированное окружение для инференса.

- **Интеграция с OpenWebUI и вывод в интернет**  
  OpenWebUI был развернут как бесплатный и удобный интерфейс для взаимодействия с LLM, позволяя управлять пользователями, настраивать API и использовать расширенные опции через Helm-чарт. Дополнительно, применение CDN и WAF гарантирует защиту и высокую скорость доступа, скрывая реальный IP кластера и обеспечивая базовую защиту от DDoS и других угроз.

В итоге, статья демонстрирует, как можно собрать современную платформу для распределённого инференса, которая не только эффективно использует аппаратные ресурсы (несколько GPU), но и обеспечивает безопасный, масштабируемый и удобный доступ к языковым моделям через веб-интерфейс. Это решение открывает широкие возможности для дальнейших экспериментов, разработки и внедрения в различных масштабах – от домашних лабораторий до крупных корпоративных инфраструктур.

Я уверен, что этот проект вдохновит вас на новые идеи и эксперименты, а полученная платформа станет отличной отправной точкой для реализации самых смелых задумок в области искусственного интеллекта и распределённых вычислений.
